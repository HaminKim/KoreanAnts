...
export default function FlowPage() {
  ...
  const rangeText = useMemo(() => {
    ...
    return `${formatDateDot(rows[0].date)} ~ ${formatDateDot(
  ...
  const histData = useMemo(
    () =>
      rows.map((r) => ({
        ...
        color:
          r.net >= 0
            ? 'rgba(239,68,68,0.65)'     // 빨강
            : 'rgba(56,189,248,0.70)',  // 하늘색
      })),
    [rows]
  );
    ...
    <>
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        <div className="space-y-1">
          ...
          {rangeText && (
            <div className="text-sm text-gray-500">기간: {rangeText}</div>
          )}
          {last && (
      ...
      <div className="flex gap-2 mb-3">
        <button onClick={() => setShowMA5((v) => !v)}
          className={`px-3 py-1 rounded border text-sm ${showMA5 ? 'bg-gray-700 text-white' : 'text-gray-400'}`}>
          MA5
        </button>
        <button onClick={() => setShowMA10((v) => !v)}
          className={`px-3 py-1 rounded border text-sm ${showMA10 ? 'bg-gray-700 text-white' : 'text-gray-400'}`}>
          MA10
        </button>
        <button onClick={() => setShowMA20((v) => !v)}
          className={`px-3 py-1 rounded border text-sm ${showMA20 ? 'bg-gray-700 text-white' : 'text-gray-400'}`}>
          MA20
        </button>
      </div>
      ...
      <div className={`w-full rounded-lg border ${themeColor.border} p-4`}>
        ...
        {err && (
          <div className="text-sm text-red-500 mb-2">에러: {err}</div>
        )}
